...
async def remove_cyberbullying(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_cyberbullying(message.text):
        ...
        if has_hit_limit(sender_id, chat_id):
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text=f"The message you've sent is a abusive, and you've exceeding the cyberbullying limit, you'll be banned from the group chat for {NO_BANNED_DAYS} days!!",
            ...
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
                text="The message you've sent is a abusive, be careful or you'll be removed from the group chat soon!",
                reply_to_message_id=message.message_id,
            )
...
if __name__ == "__main__":
    ...
    cyberbullying_handler = MessageHandler(
        filters.TEXT & filters.ChatType.GROUPS, remove_cyberbullying
    )  # filters.TEXT & filters.ChatType.GROUPS tells us we want only text messages from group chats
